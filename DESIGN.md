...
- **Panel Surface** (rgba(236, 236, 236, 0.55) + blur): A translucent frosted fill for the three right-sidebar cards; it sits lighter on the white panel than a solid gray block while still separating work categories.
...
Changes, Plan, and Background Tasks remain three separate cards because they represent distinct operational concerns. Each uses a translucent Panel Surface (`rgba(236,236,236,0.55)`) with a 18px backdrop blur, a 12px radius, an 11–12px inset, and a neutral hairline with no shadow. Lists inside stay borderless and linear so the categories read as cards without becoming nested card stacks.
