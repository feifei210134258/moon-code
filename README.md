...
Kimi Agent Desktop 是一个以 Kimi Code 为唯一 Agent 内核的本地桌面客户端。
...
项目已进入 Foundation 实现阶段：主工作台视觉基线、Electron + Vue 工程、托管 Kimi 运行时、typed IPC，以及 0.29.0 REST/WebSocket 契约适配层均已落地。Spike A/B 仍在继续验证，当前版本还不是可替代官方 Web 的 Beta。
...
- 当前有 193 个可运行的单元/组件用例与一项 opt-in 真实 Kimi Runtime 集成测试覆盖主链路；生产构建、依赖审计和 arm64 `.app` 内的 native PTY/Browser smoke 均纳入门禁。签名、公证与发布安装包仍属于 Beta 发布阶段。
