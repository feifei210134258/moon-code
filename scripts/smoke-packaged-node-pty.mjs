...
const child = spawn(executable, ['--smoke-node-pty'], {
  cwd: process.cwd(),
...
child.stderr.on('data', (chunk) => { output = `${output}${chunk.toString('utf8')}`.slice(-32_768) })

const result = await new Promise((resolveResult, reject) => {
  const timer = setTimeout(() => {
    child.kill('SIGKILL')
    reject(new Error(`Packaged Kimi Agent smoke timed out:\n${output}`))
  }, 20_000)
...
if (result.code !== 0 || !output.includes(marker)) {
  throw new Error(`Packaged node-pty smoke failed (${result.signal ?? result.code}):\n${output}`)
}
