...
const child = spawn(executable, ['--smoke-browser'], {
  cwd: process.cwd(),
...
const result = await new Promise((resolveResult, reject) => {
  const timer = setTimeout(() => {
    child.kill('SIGKILL')
    reject(new Error(`Packaged Browser smoke timed out:\n${output}`))
  }, 30_000)
...
if (result.code !== 0 || !output.includes(marker)) {
  throw new Error(`Packaged Browser smoke failed (${result.signal ?? result.code}):\n${output}`)
}
